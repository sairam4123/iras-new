...
def get_speech_client() -> tts.TextToSpeechClient:
    tts_client: tts.TextToSpeechClient = (
        tts.TextToSpeechClient.from_service_account_info(
...
def select_voice_people(lang: str) -> dict[str, tts.Voice]:
    ...
    gender_voices: dict[
        Literal["male"] | Literal["female"] | Literal["neutral"], list[tts.Voice]
    ] = {
        "male": [],
        "female": [],
        "neutral": [],
    ...
    available = [
        v
        for v in gender_voices.get(
            person_gender if person_gender in gender_voices else "neutral", []
        )
    ]
    available = [
        v for v in available if all(ign not in v.name for ign in ignored_voices)
    ]
    ...
    selected_voices = [
        available for available in available
    ]  # if 'Vindemiatrix' in available.name]
    if selected_voices:
        voice = selected_voices[1]
    else:
...
def t2s(text: str, out_fp: BytesIO | BufferedRandom, lang: str = "en-IN"):
    ...
    voice_params = tts.VoiceSelectionParams(
        language_code=lang, name=voice.name, ssml_gender=voice.ssml_gender
    )
