...
class ETrainAPIAsync:
        ...
        async with self.session.post(
            ...
            try:
                ...
                if "captcha" in json.get("sscript", {}):
                    ...
                    if not curr_cookie:
                        raise ETrainAPIError("failed to get new session token")
    ...
    async def authenticate(self, json_resp):
        ...
        error = captcha_soup.find("span", attrs={"id": "captchaformerrormsg"})
        if error:
        ...
        key = await self.captcha_handler(
            encoded_hash, keys, str(error), str(CAPTCHA_FOLDER / captcha_file)
        )
        try:
